...
def random_crop(x_i):
    idx0 = rng.random_integers(low=0, high=200, ndim=1)
    idx1 = rng.random_integers(low=584, high=784, ndim=1)
...
# Prediction
n_imgs = 100
e = np.random.normal(0, 1, (n_imgs, n_z))
crop_teX = crop(teX[0:n_imgs])
mask_teX, pred_teX = predict(crop_teX, e)
...
scipy.misc.imsave('./predictions/vae.gif', img_array)
