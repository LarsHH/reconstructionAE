...
# Loss
reconstruction_L = -T.dot(reconstruction_dims.T, X * T.log(X_hat) + (1 - X) * T.log(1 - X_hat))
prediction_L = -T.dot(prediction_dims.T, X * T.log(X_hat) + (1 - X) * T.log(1 - X_hat))
...
# Compiling
train = theano.function(inputs=[X], outputs=[loss, T.mean(reconstruction_L), T.mean(prediction_L), X_hat[-2:]],
                        updates=updates, allow_input_downcast=True)
predict = theano.function(inputs=[X], outputs=[model_input, X_hat, T.mean(reconstruction_mse), T.mean(prediction_mse)],
...
# Prediction
n_imgs = 100
mask_teX, pred_teX, reconstruction_error, prediction_error = predict(teX)
